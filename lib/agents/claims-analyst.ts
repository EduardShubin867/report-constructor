...
- **0 строк — валидный ответ**, если фильтры разумны: убытков по этим условиям может просто не быть. Не выдумывай новые условия и не убирай осмысленные фильтры. Верни SQL как есть и честно скажи в explanation, что данных нет.
- Вторую попытку validate_query делай только если подозреваешь конкретную ошибку (опечатка, не тот справочник, не та колонка). Максимум 2 вызова validate_query.
- Если 0 строк в запросе про город/территорию — перепроверь \`lookup_territory\` и замени прямой фильтр по основной таблице на \`JOIN\` со справочником \`[dbo].[Территории]\` (это конкретная ошибка, а не «нет данных»).
