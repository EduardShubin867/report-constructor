...
export default function SourceEditorForm({
  ...
  async function handleGenerateWhenToUse() {
    ...
    try {
      const res = await fetch('/api/admin/sources/generate-when-to-use', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          name: form.name,
          draft: form.whenToUse,
          tables: form.tables.split('\n').map(t => t.trim()).filter(Boolean),
        }),
      });
      ...
      onFieldChange('whenToUse', data.whenToUse);
    } catch (err) {
        ...
        <div className="block">
          <div className="mb-1 flex items-center justify-between gap-2">
            ...
            <Button
              ...
              disabled={isIntrospecting || isGenerating || (!form.name.trim() && !form.whenToUse.trim())}
          ...
          <textarea
            ...
            disabled={isIntrospecting || isGenerating}
          />
